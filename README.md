...
Since each climate model produces slightly different results — and following statistician George E. P. Box’s principle that “all models are wrong, but some are useful” — it is not appropriate to rely on a single model to represent future climate. Each model contributes its own perspective and uncertainty. Therefore, rather than selecting one or some of the models, this project combines all available models within a Bayesian hierarchical framework, allowing every model to inform the results while formally quantifying structural differences and uncertainties.

> ## Raw data availability

The raw climate datasets used in this project (HadUK-Grid & UKCORDEX daily tasmax/tasmin) are extremely large (tens of GB) and are distributed under the CEDA licence.
